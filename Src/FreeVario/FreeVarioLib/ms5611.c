...
void MS5611_readPressure(I2C_HandleTypeDef *hi2c,SD_MS5611* MS_Datastruct){
	...
	MS5611_sendCommand(CMD_CONV_D1_BASE+OSR*CONV_REG_SIZE,MS_Datastruct->adress, hi2c);	//read sensor, prepare a data
...
void MS5611_readTemperature(I2C_HandleTypeDef *hi2c, SD_MS5611* MS_Datastruct){
	...
	//****************
	if(abs(HAL_GetTick() - MS_Datastruct->_lastTime)<T_THR)
	 	return;
	MS_Datastruct->_lastTime = HAL_GetTick();
	//****************
	MS5611_sendCommand(CMD_CONV_D2_BASE+OSR*CONV_REG_SIZE,MS_Datastruct->adress, hi2c);		//read sensor, prepare a data
	...
	MS_Datastruct->_T = 2000 + ((int64_t)MS_Datastruct->_dT * MS_Datastruct->uC[6-1])/8388608;

}

void MS5611_readCalibration(I2C_HandleTypeDef *hi2c, SD_MS5611* MS_Datastruct){
	for(uint8_t k=0;k<6;k++){
		MS5611_sendCommand(CMD_PROM_READ_BASE + k*2,MS_Datastruct->adress, hi2c);
		MS_Datastruct->uC[k] = (uint16_t) (MS5611_readnBytes(NBYTES_PROM ,MS_Datastruct->adress, hi2c) & 0xFFFF); //masking out two LSB
	}
}
...
void MS5611_sendCommand(uint8_t cmd, uint8_t adress, I2C_HandleTypeDef *hi2c){
	...
	Buffer_Tx1[0] = cmd;
	if( HAL_I2C_Master_Transmit(hi2c,(uint16_t)adress, (uint8_t *)Buffer_Tx1,1,100)!= HAL_OK) {
...
uint32_t MS5611_readnBytes(uint8_t nBytes, uint8_t adress, I2C_HandleTypeDef *hi2c){
	...
	if( HAL_I2C_Master_Receive(hi2c,(uint16_t)(adress + 1), (uint8_t *)Buffer_Rx,nBytes,100)!= HAL_OK) {
