...
void NMEA_setGforce(float gforce) { //$FBG does not exist, unable to find a gforce nmea sentence
...
void NMEA_setnmeaShortLXWP0(float varioAlt, float varioMts) { //in m/s
  ...
  sprintf(nmeaVarioLXWP0,"%s%5.2f%s%2.2f%s","$LXWP0,N,,",varioAlt,",",varioMts,",,,,,,,,*");
...
void NMEA_setnmeaVarioLXWP0(double varioAlt, float a, float b, float c, float d, float e, float f) { //in m/s
  // $LXWP0,logger_stored, airspeed, airaltitude,
  //   v1[0],v1[1],v1[2],v1[3],v1[4],v1[5], hdg, windspeed*CS<CR><LF>
  //
  // 0 loger_stored : [Y|N] (not used in LX1600)
